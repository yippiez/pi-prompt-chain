...
// Outline graph glyphs (jjui-style): circle nodes + branching connector lines.
export const NODE_OPEN = "○"; // leaf or expanded node
export const NODE_FILLED = "●"; // collapsed node (has hidden children)
export const BRANCH = "│ "; // continuing vertical connector
export const BRANCH_BLANK = "  "; // empty branch column
...
// than `width` columns. Used for horizontal scrolling of fixed-width output.
export function sliceDisplayWidth(text: string, startCols: number, width: number): string {
	let col = 0;
	let out = "";
	for (const ch of text) {
		const cw = visibleWidth(ch) || 1;
		if (col + cw <= startCols) {
			col += cw;
			continue;
		}
		if (visibleWidth(out) + cw > width) break;
		out += ch;
		col += cw;
	}
	return out;
...
export function shortcutsText(width: number, thm: ExtensionContext["ui"]["theme"]): string {
	const parts = ["⇥ indent", "⌥⇧↑/↓ move", "⌥↑/↓ hist", "^⌫ del", "^␣ fold", "@ files", "^s run", "^f path", "^l lang", "Pg scroll", "⌥←/→ xscroll", "^r send", "^t think", "^o tools"];
